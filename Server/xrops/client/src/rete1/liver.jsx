...
export class LiverControl extends Rete.Control {
  ...
  constructor(emitter, key, node, readonly = false) {
    ...
    this.key = key;
    this.component = LiverControl.component;
    node.id =
...
export class LiverComponent extends Rete.Component {
  constructor(type) {
    super(`${type}`);
  ...
  builder(node) {
    ...
    var ctrl = new LiverControl(this.editor, "data", node);
    node.id =
      Date.now().toString() + (Math.random() + 1).toString(36).substring(7);
