...
export class FilterControl extends Rete.Control {
  ...
  constructor(emitter, key, node, readonly = false) {
    ...
    this.key = key;
    this.component = FilterControl.component;
...
export class FilterComponent extends Rete.Component {
  constructor(type) {
    super(`${type}`);
  ...
  builder(node) {
    ...
    var ctrl = new FilterControl(this.editor, "image", node);
    node.id =
      Date.now().toString() + (Math.random() + 1).toString(36).substring(7);
  ...
  async worker(node, inputs, outputs) {
    ...
    var data1 = {
      ...
      mode:
        this.filter_type === "eosin" || this.filter_type === "nuclei"
          ? "P"
          : "",
    };
    var data2 = {
      storageFilePath: file["storage_file_path"],
      inputType: file["file_type"].replace(/\_downscaled/g, ""),
      filterType: this.filter_type,
      userId: "kuvis_user1",
      nodeId: node.id,
      nodeType: "filter",
