...
const Explorer = (props) => {
  ...
  async function getText(){
    ...
    text = await XRGetTextAPI("/workspace/xrops/users/"+p);
    setText(text);
  }

  async function getFileList(){
    const data = await getFilesAPI(dir,'.csv,.json');
    ...
    if(fileList[i]===file){
      file_list_html.push(
        <div style={{display: "flex", flexDirection: "column", alignItems: "center"}}>
          <div id={fileList[i]} className="file-snapshot selected" onClick={changeFile} style={{backgroundImage: "url(data.png)"}}></div>
      ...
      file_list_html.push(
        <div style={{display: "flex", flexDirection: "column", alignItems: "center"}}>
          <div id={fileList[i]} className="file-snapshot" onClick={changeFile} style={{backgroundImage: "url(data.png)"}}></div>
          <p className="file-text">{fileList[i]}</p>
        </div>
      );
    }
